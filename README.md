...
默认审美风格是**研究档案式编辑设计**。如果你没有指定其他风格，它会自动使用纸白、冷灰、理性蓝、真实材料层次、克制留白、研究工作台、轻微颗粒和少量手写批注的视觉系统。
...
```text
Use $ado-visual-director 直接生图：把“AI医学产品真正重要的是责任链”做成一张PPT配图，使用ppt-slide预设。
```

没有指定审美风格时，它会默认使用“研究档案式编辑设计”。
...
```text
Use $ado-visual-director 直接生成图片：
主题：AI辅助诊断不是替代医生，而是让责任链更清楚。
平台：小红书
页数：4页
尺寸：1080x1350
```

直接生图时会先做视觉导演方案，再用最终提示词调用当前环境可用的图片生成工具。多页图片会按页码顺序生成。

如果当前环境不能调用图片生成工具，它会说明无法直接出图，并输出可复制到生图工具里的最终提示词包。

## 解释案例：PPT 配图从提示词到直接生图

这个案例展示同一个主题如何先生成视觉导演方案，再进一步直接生成图片。
...
第一步，调用 `ado-visual-director` 生成默认提示词和视觉导演方案。没有指定审美风格时，skill 会自动使用“研究档案式编辑设计”，并套用 `ppt-slide` 的 `1920x1080 / 16:9` 规格。

![默认生成提示词示例](assets/examples/default-prompt-workflow.png)

第二步，继续要求“帮我直接生成图片”。skill 会基于同一套视觉导演方案进入直接生图流程，生成 PPT 演示页配图。

![直接生成图片操作记录](assets/examples/direct-image-workflow.png)

最终生成图示例：

![AI医学责任链PPT配图](assets/examples/generated-ppt-responsibility-chain.png)

这个案例里可以看到三个默认规则都生效了：

- `ppt-slide`：输出 16:9 横版 PPT 配图。
- 默认研究档案式：纸白、冷灰、理性蓝、真实材料层次、少量手写批注。
- AI+医学模式：强调医生确认、责任链、随访闭环，避免机器人医生、AI 大脑、赛博医院。
...
没有指定风格时，默认使用“研究档案式编辑设计”：

```text
纸白、冷灰、理性蓝、克制黑色标题、真实工作材料、证据层次、研究工作台、半透明数字界面、蓝色结构线、少量手写批注、轻微颗粒质感。
```
...
```text
审美风格：更像展览海报，黑白为主，少量红色批注，但仍保留研究档案式的证据层次。
```
...
如果 `visual_plan.json` 没有写 `style_system`，脚本会自动补上研究档案式默认风格。
...
如果你只是要视觉方案，它不会直接生成图片；如果你明确说“直接生图、生成图片、出图”，它会尝试调用当前环境可用的图片生成工具。当前环境不能生图时，它会输出最终提示词包。
